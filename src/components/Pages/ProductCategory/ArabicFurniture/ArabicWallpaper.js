...
const ArabicWallpaper = () => {
	const [limit, setLimit] = useState(4);
	const [page, setPage] = useState(1);

	const getFacts = () => {
		const res = axiosInst.get(`/product?category=arabic&type=wallpaper&page=${page}&limit=${limit}`).then((response) => {
			return response
		})
		return res;
	};
	...
	return (
		<div className='min-h-screen'>
			<div className=''>
				<h1 className='capitalize text-3xl text-gray-900 text-center my-5 font-semibold'>Arabic Wallpaper</h1>
...
export default ArabicWallpaper;